...
impl Book {
    ...
    pub fn new(abbrev: String, title: String, chapters: Vec<Chapter>) -> Self {
        Book { abbrev, title, chapters }
    }

    /// Returns the book's abbreviation.
    pub fn abbrev(&self) -> &str { &self.abbrev }
    ...
    pub fn get_chapter(&self, chapter_number: usize) -> Option<&Chapter> {
        self.chapters.iter().find(|c| c.chapter_number == chapter_number)
    }
    ...
    pub fn get_verse(&self, chapter_number: usize, verse_number: usize) -> Option<&Verse> {
        self.get_chapter(chapter_number).and_then(|c| c.get_verse(verse_number))
    }
}
...
impl Bible {
    ...
    pub fn get_book_by_abbrev(&self, abbrev: &str) -> Option<&Book> {
        self.index_by_abbrev.get(abbrev).and_then(|&i| self.books.get(i))
    }
    ...
    pub fn get_verses(&self, book: BibleBook, chapter_number: usize) -> Option<&Vec<Verse>> {
        self.get_book(book).and_then(|b| b.get_verses(chapter_number))
    }
    ...
    pub fn get_verse(
        ...
        self.get_book(book).and_then(|b| b.get_verse(chapter_number, verse_number))
    }
            ...
            let chapters = entry
                ...
                .map(|(chapter_idx, verses)| {
                    let verses = verses
                        .iter()
                        .enumerate()
                        .map(|(verse_idx, verse_text)| Verse::new(verse_text.clone(), verse_idx + 1))
                        .collect();
...
mod tests {
    ...
    fn create_test_book() -> Book {
        let chapters = vec![
            create_test_chapter(),
            Chapter::new(vec![
                Verse::new("Thus the heavens and the earth were finished.".to_string(), 1),
    ...
    fn test_verse_creation() {
        ...
        assert_eq!(verse.verse_text, "In the beginning God created the heaven and the earth.");
    ...
    fn test_verse_display() {
        ...
        assert_eq!(display, "1: In the beginning God created the heaven and the earth.");
    }
    ...
    fn test_bible_new_from_map() {
        ...
        map.insert("gn".to_string(), FileDataEntry {
